...
import { jsonWithSetCookies, noContentWithSetCookies } from '@/infra/http/set-cookie-response';
...
export function createPublicRoutes(container: AppContainer) {
  ...
  return new Elysia({ prefix: '/api' })
    ...
    .post('/login', async ({ body }) => {
      ...
      return jsonWithSetCookies({ user: result.user }, result.setCookieHeaders);
    }, {
      detail: {
        tags: ['auth'],
    ...
    .post('/auth/refresh', async ({ headers }) => {
